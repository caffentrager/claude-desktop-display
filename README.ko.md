...
- 모멘터리 푸시 버튼 (선택 사항 - 화면을 수동으로 넘길 때 씀. 없어도
  자동 로테이션만으로 잘 동작한다)
- 점퍼 와이어
...
버튼(선택 사항): 한쪽 다리는 **GPIO7**, 다른 쪽은 **GND**로. 내부 풀업을
쓰기 때문에 저항은 필요 없다. 보드에 GPIO7이 안 나와 있다면 `main.cpp`의
`PIN_SCREEN_BUTTON`을 SDA/SCL이나 스트래핑 핀(ESP32-C3의 GPIO2/8/9)이
아닌 다른 빈 핀으로 바꾸면 된다.
...
화면은 몇 초 간격으로 (`main.cpp`의 `SCREEN_ROTATE_MS`) 세 화면을 순서대로
...
(`▢`는 빈 칸이 아니라 테두리만 있는 "빈 슬롯" 문자다 - 0%일 때도 막대의
경계가 보이게. 실제 막대 해상도는 칸 단위보다 더 세밀하다 - 아래 참고.)
...
- 세 번째 "한눈에 보기" 화면은 둘 다 압축해서 한 화면에 보여준다 - 공간이
  없어서 막대그래프도, stale(`!`) 표시도 없이 숫자만 나온다. 그게
  필요하면 각각의 전용 화면을 보면 된다.
- 막대 자체도 한 칸 단위보다 더 세밀한 해상도를 쓴다 (`main.cpp`의
  `BAR_SUBDIV`, 칸당 5단계) - 커스텀 문자 몇 개를 추가로 써서 꽉 찬
  칸/빈 칸만 있는 게 아니라 부분적으로 채워진 칸도 표현한다. 빈 칸도
  완전히 비워두지 않고 테두리를 그려서, 0%일 때도 막대 전체 폭이 보인다.
