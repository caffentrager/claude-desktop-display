...
- **PCF8574 I2C 백팩**이 달린 캐릭터 LCD - 18x2, 16x2, 20x4 다 되고,
  `firmware/src/main.cpp`의 `LCD_COLS`/`LCD_ROWS` 참고
- 모멘터리 푸시 버튼 1~2개 (둘 다 선택 사항):
  - 화면 버튼 - 화면을 수동으로 넘길 때 씀. 없어도 자동 로테이션만으로
    잘 동작한다
  - 디버그 버튼 - 화면에서 바로 실행되는 자가 테스트를 돌림 (아래
    [디버그 모드](#디버그-모드) 참고). 없어도 무방하다
- 점퍼 와이어
...
상태가 표시된다. LCD가 18x2가 아니라면 `firmware/src/main.cpp` 맨 위의
...
해상도는 칸 단위보다 더 세밀하다 - 아래 참고. 2번째 줄 끝의 ✳는 나머지
내용을 다 쓰고도 자리가 남을 때만 붙는다 - 5H 화면에서는 바로 붙여쓰고,
WK 화면에서는 두 칸 띄운다. 둘 다 한 화면에 보여주는 세 번째 "한눈에
보기" 화면도 코드에는 있지만 지금은 꺼져 있다 - `main.cpp`의
`SCREEN_COUNT`.)
...
- 2번째 줄 끝에 붙는 `!`는 기기가 한동안 제대로 된 값을 못 받아왔다는
  뜻이다 (WiFi 끊김, API 타임아웃 등) - 화면에는 마지막으로 알던 값이 그대로
  남는다.
...
디버그 버튼(GPIO21)이 연결되어 있다면, 누르면 실제 렌더링 코드를 그대로
써서 약 25초짜리 화면 자가 테스트가 돌아간다 - 실제 사용량 데이터나
달력/시계가 모든 값을 자연스럽게 거쳐갈 때까지 기다릴 필요가 없다:

1. 막대를 0부터 99까지 전 구간 훑으면서 채움/부분채움/빈칸 글리프를
   눈으로 확인할 수 있다. 0%에서 3초간 멈춰서 그 화면부터 제대로 보이게
   한 다음 움직이기 시작한다.
2. 요일 라벨 7개(월~일)를 고정된 시각으로 순서대로 보여줘서 요일 표기가
   맞는지 확인할 수 있다.
3. 시계 표시를 00:00부터 24:00까지 한 시간 단위로 훑어서 두 자리 시(時)
   표기가 다 맞는지 확인할 수 있다 (24:00은 실제로 그렇게 나온다는 뜻이
   아니라, 화면 스윕의 끝점으로 포함시킨 것).
4. 분 표시를 00부터 59까지 훑어서(시는 0으로 고정) 두 자리 분(分) 표기도
   확인할 수 있다 - 위 시(時) 스윕에서는 분이 계속 ":00"으로만 나왔었다.

끝나면 자동으로 원래 화면으로 돌아온다 - 아무것도 저장하거나 바꾸지
않는, 디스플레이에 대해서만 읽기 전용인 테스트다.
