...
버튼(선택 사항): 한쪽 다리는 **GPIO6**, 다른 쪽은 **GND**로. 내부 풀업을
쓰기 때문에 저항은 필요 없다. 보드에 GPIO6이 안 나와 있다면 `main.cpp`의
...
처음 전원을 넣으면 WiFi 연결을 시작하기 전에 잠깐 "ClaudeMeter ✳" 스플래시가
뜬다 (✳는 작은 커스텀 반짝임 문자이고, 실제 로고를 재현한 건 아니다).
